...
class TenexCLI:
    ...
    def add_liquidity(self, amount: str, network: str):
        ...
        try:
            ...
            w3, account = TenexUtils.get_signer_for_miner(network)
            contract_address = TenexUtils.get_tenexium_protocol_address(network)
            contract = TenexUtils.get_contract("addLiquidity", w3, network)
            ...
            # Check LP info
            lp_info = TenexUtils.get_contract("liquidityProviders", w3, network).functions.liquidityProviders(account.address).call()
    ...
    def remove_liquidity(self, amount: str, network: str):
        ...
        try:
            ...
            w3, account = TenexUtils.get_signer_for_miner(network)
            contract_address = TenexUtils.get_tenexium_protocol_address(network)
            contract = TenexUtils.get_contract("removeLiquidity", w3, network)
            amount_wei = w3.to_wei(amount, 'ether')
    ...
    def show_liquidity_stats(self, address, w3: Web3, network: str):
        ...
        try:
            total_lp_stakes = w3.from_wei(TenexUtils.get_contract("totalLpStakes", w3, network).functions.totalLpStakes().call(), 'ether')
            total_lp_fees = w3.from_wei(TenexUtils.get_contract("totalLpFees", w3, network).functions.totalLpFees().call(), 'ether')
            lp_info = TenexUtils.get_contract("liquidityProviders", w3, network).functions.liquidityProviders(address).call()
            liquidity_circuit_breaker = TenexUtils.get_contract("liquidityCircuitBreaker", w3, network).functions.liquidityCircuitBreaker().call()
...
Examples:
  ...
  python3 tenex.py showstats --network <network>
    ...
    parser.add_argument(
        "command",
        choices=["addliq", "removeliq", "showstats"],
    ...
    if args.command == "addliq":
        ...
        w3, account = TenexUtils.get_signer_for_miner(args.network)
