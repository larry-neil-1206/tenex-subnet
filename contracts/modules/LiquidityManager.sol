...
abstract contract LiquidityManager is TenexiumStorage, TenexiumEvents {
		...
		internal
	{
		...
		uint256 newAvailableLiquidity = availableLiquidity > withdrawAmount ? 
			availableLiquidity - withdrawAmount : 0;
		...
		uint256 newUtilizationRate = newTotalLp > 0 ? 
			totalBorrowed.safeMul(PRECISION) / newTotalLp : 0;
	 ...
	 */
	function getUtilizationStats(uint16 alphaNetuid) external view returns (
		uint256 utilizationRate,
		uint256 borrowingRate,
		uint256 totalCollateralPair,
