	...
	PrecompileUtils
{
	...
	function _executeBuyback() internal {
		...
		if (address(this).balance < buybackAmount) revert TenexiumErrors.InsufficientContractBalance(address(this).balance, buybackAmount);
		...
		uint256 actualSlippage = expectedAlpha > actualAlphaReceived ? 
			((expectedAlpha - actualAlphaReceived) * 10000) / expectedAlpha : 0;
