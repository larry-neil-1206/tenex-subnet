...
contract AddressConversion is BLAKE2_Constants {
    ...
    // Mixing Function
    function G(uint64[16] memory v, uint256 a, uint256 b, uint256 c, uint256 d, uint64 x, uint64 y) public pure {
    ...
    function compress(BLAKE2bContext memory ctx, bool last) internal view {
    ...
    function update(BLAKE2bContext memory ctx, bytes memory input) internal view {
    ...
    function finalize(BLAKE2bContext memory ctx, uint64[4] memory out) internal view {
        // uint256 i;
    ...
    function blake2b(bytes memory input, uint64 outlen) public view returns (uint64[4] memory) {
    ...
    function getWords(uint64 a) public pure returns (uint64 b) {
        return ((a & MASK_0) / SHIFT_0) ^ ((a & MASK_1) / SHIFT_1) ^ ((a & MASK_2) / SHIFT_2) ^ ((a & MASK_3) / SHIFT_3)
            ^ ((a & MASK_4) * SHIFT_3) ^ ((a & MASK_5) * SHIFT_2) ^ ((a & MASK_6) * SHIFT_1) ^ ((a & MASK_7) * SHIFT_0);
    }
